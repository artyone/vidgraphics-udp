...
from PyQt5.QtWidgets import (QAction, QApplication, QComboBox, QHBoxLayout,
                             ...
                             QPushButton, QSlider, QStyle, QToolBar, QSplitter,
...
class MainData:
    ...
    def unpack_data(self, res):
        ...
        for index, (name, coef) in enumerate(zip(main_headers, main_coef)):
            self.add_data(name, res['data_main'][:,index] * coef)
...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        self.setWindowTitle("VID GRAPH UPD v.2024.02.07")
        ...
        right_widget.setStyleSheet('QWidget {background-color: rgb(0, 0, 0)}')
...
class GraphWidget(pg.PlotWidget):
    ...
    def mouse_moved(self, ev):
        if self.sceneBoundingRect().contains(ev):
            ...
            self.setToolTip(
                f'<b>{self.pos_y:.3f}</b>' #x: <b>{self.pos_x:.2f}</b>,<br> y: 
            )
...
class VidGraph(pg.PlotWidget):
    ...
    def create_graphs(self):
        ...
        curve = pg.PlotDataItem(ox, oy, name='vid_data', pen=pen, connect='all')
        self.addItem(curve)
        self.curve = curve
